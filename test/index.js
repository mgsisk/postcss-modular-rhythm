...
function testBattery(testData) {
  const regex = new RegExp('(\\S+)/(\\S+) c', 'u')
...
postcss([plugin]).process(data, {from: undefined}) // eslint-disable-line no-undefined
  .then((result)=> {
    ...
    tap.match(result.css, /font: .+\/1\.04167 b;/u, 'It scales mlh values up')
    tap.match(result.css, /font: .+\/2\.16 c;/u, 'It scales mlh values down')
    tap.match(result.css, /font: 5\.15978em\/1\.16284 d;/u, 'It ensures a minimum mlh value of 1')
...
postcss([plugin]).process(rootData, {from: undefined}) // eslint-disable-line no-undefined
  .then((result)=> {
    ...
    tap.match(result.css, /font: .+\/1\.03125 a;/u, 'It uses the correct mlh value with custom bases')
